...
export const projects = {
    ...
    members: [
      {
        ...
        role: ['프론트엔드', '백엔드'],
        description:
          '첫 프론트엔드 프로젝트로 서비스를 기획하고 구현하면서, 머릿속으로 그리던 화면과 사용자 흐름이 실제로 동작하는 경험이 특히 뜻깊었습니다. 직접 UI를 구성하고 API 연동까지 연결해보며 프론트엔드의 재미와 어려움을 동시에 체감했습니다. 작은 디테일 하나가 사용성에 큰 영향을 준다는 것도 몸소 느꼈고, 문제를 하나씩 해결해가며 UI/UX가 점점 개선되는 과정을 볼 때마다 큰 뿌듯함을 얻었습니다. 이번 개인 프로젝트를 통해 기획부터 개발, 연결과 검증까지 서비스 제작의 전체 플로우를 경험할 수 있었던 의미 있는 시간이었습니다.',
        profileImage: KimDoyeonImg,
      },
    ],
  },
    ...
    members: [
      {
        ...
        role: ['PM', '프론트엔드', '백엔드'],
        description:
          '첫 개인 프로젝트로서 기획부터 시작해 프론트, 백을 모두 구현해보는 뜻깊은 경험을 하였습니다. 새로운 기술들을 배우고 프로젝트에 적용하였는데, 특히 익명 사용자 세션 관리와 Firebase 기반의 실시간 데이터 처리를 구현하는 데 신경썼습니다. 또한, 프로젝트의 주 기능인 타입별 스크랩 기능은 React의 비동기 로직을 활용하여 구축하고, 유연한 카테고리 관리 시스템을 직접 설계하며 기술적 성장을 이룰 수 있었습니다. 개발 및 배포 과정에서 수많은 오류들이 생겼었지만 끈기 있게 해결하고자 했고, 제 아이디어를 하나의 웹 서비스로 구축하는 경험은 개발자로서 큰 자산이 될 것입니다.',
        profileImage: JeongJieunImg,
      },
    ],
  },
    ...
    members: [
      {
        ...
        role: '프론트엔드',
        description:
          'APPS에서 신입 부원 프로젝트로서 CommuTime-통학·통근 자기계발 서비스 개발을 진행했습니다. 등교하는 시간을 어떻게 하면 더 알차게 보낼 수 있을지 고민하다 생각해낸 서비스입니다. 처음 생각했던 서비스 기능들보다 현저히 적게 나와 많이 아쉽지만 이 개인 프로젝트로 많이 배울 수 있었습니다. 이 경험을 바탕으로 앞으로 더 기발하고 의미 있는 프로젝트를 진행하고 싶습니다 ✊✊',
        profileImage: KimYoungkyoImg,
      },
    ],
  },
    ...
    members: [
      {
        ...
        role: '프론트엔드',
        description:
          '처음으로 리액트 프로젝트를 진행해보며 html, css, js만 사용했을 때보다 더 실전적인 개발에 다가간 것 같아 뿌듯합니다. 프로젝트 주제를 정할 때 그동안 배운 기초적 기술이 들어가면서도 남들과는 다른 아이디어를 내보고 싶었는데 어느 정도 성공한 것 같아 다행이라고 생각합니다. 뷰티가 주제인 만큼 예쁜 CSS를 적용하고 싶었는데 기대만큼은 구현하지 못한 것 같아 아쉽고, 시간이 부족해서 다른 기능을 더 추가하지 못해 아쉽지만 그래도 개발 과정에서 크게 성장한 것 같습니다. 이번에는 프론트엔드로만 프로젝트를 구현했지만 다음에는 백도 직접 공부해서 더 퀄리티 높은 프로젝트를 진행해보고 싶어요!',
  ...
  timetablemaker: {
    ...
    feature: [
      '📘 자동으로 충돌 없는 시간표 만들기\n수강하고 싶은 과목을 선택하면, 해당 과목들을 조합해 충돌하지 않는 시간표를 생성합니다.',
      '📘 나만의 시간표 미리보기\n생성된 시간표를 주간 형태로 미리 확인하며, 각 요일별 강의 시간과 강의실을 시각적으로 쉽게 파악할 수 있습니다.',
      '📘 조건 기반 시간표 추천\n공강 요일, 점심 시간 확보, 하루 최대 수업 수 등 원하는 조건을 설정하여 더욱 자신에게 맞는 시간표를 추천받을 수 있습니다.',
    ],
    techStack: ['React'],
    links: [
      {type: 'CLIENT', url: 'https://github.com/echae0111/timetable-maker'},
      {type: 'LINK', url: 'https://echae0111.github.io/timetable-maker/'},
    ],
    members: [
      {
        name: '이은채',
        role: ['프론트엔드', '백엔드'],
        description:
          '이번 프로젝트를 무사히 마칠 수 있어 뿌듯합니다! 혼자 개발하면서 방향성도 여러번 잃고 문제가 생길 때마다 해결해 나가는 과정이 쉽진 않았지만 그만큼 많은 것을 배우는 기회가 되었습니다. 주제 선정부터 구현까지 처음 해보는 프로젝트라 걱정도 많았지만 덕분에 기술적인 역량뿐 아니라 개발에 임하는 태도와 사고방식 전반에서 한 단계 성장할 수 있었던 소중한 경험이 된 것 같습니다.',
        profileImage: LeeEunchaeImg,
      },
    ],
  },
